...
// 팀간 평가 제출 핸들러
function handleTeamEvaluationSubmit(e) {
    const form = e.target;
    ...
    // 이미 평가한 경우 수정 불가
    const myEvaluations = getTeamEvaluationsByEvaluator(user.name);
    const existingEval = myEvaluations.find(ev => ev.targetTeam === teamId);
    if (existingEval) {
...
// 팀원 평가 제출 핸들러
function handleMemberEvaluationSubmit(e) {
    const form = e.target;
    ...
    // 이미 평가한 경우 수정 불가
    const myEvaluations = getMemberEvaluationsByEvaluator(user.name, selectedDate.year, selectedDate.month);
    const existingEval = myEvaluations.find(ev => ev.targetEmployee === employeeName);
    if (existingEval) {
        alert('이미 평가를 완료했습니다. 평가는 수정할 수 없습니다.');
        return;
    }
    ...
    try {
        saveMemberEvaluation(evaluation);
