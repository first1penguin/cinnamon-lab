...
async function renderResultsPage() {
    ...
    // API에서 랭킹 조회
    const rankings = await getAllTeamRankingsAPI(selectedDate.year, selectedDate.quarter);
    ...
    for (const team of teams) {
        html += await renderTeamDetailChart(team);
    }
    ...
    for (const team of teams) {
        html += await renderTeamMembersResults(team);
    }
...
// 팀 상세 차트 렌더링
async function renderTeamDetailChart(team) {
    const selectedDate = getSelectedDate();
    const scores = await getTeamAverageScoresAPI(team.id, selectedDate.year, selectedDate.quarter);
...
// 팀별 직원 결과 렌더링
async function renderTeamMembersResults(team) {
    ...
    for (const member of members) {
        const scores = await getEmployeeAverageScoresAPI(member.name, selectedDate.year, selectedDate.quarter);
